...
const db = new Client({ connectionString: process.env.DATABASE_DIRECT_URL });
...
try {
  ...
  console.log(`\n# test user ${email}  sub=${userId}\n`);

  // 2. Sign in with the anon client to get a real access_token (HS256 JWT)
  ...
  log(body.agentId === agentId, 'Player.agentId == request agentId');
  ...
  try {
    ...
    console.log('\n# cleanup done (test user + rows removed)');
  } catch (e) {
